...
def get_base_from_ee(
    ...
    tool_from_ee = get_relative_link_pose(
        robot.robot_id, ee_link, robot.tool_link_id, robot.physics_client_id
    )
    ...
    base_from_ee = multiply_poses(
        world_from_base.invert(), world_from_target, tool_from_ee
    )
    return base_from_ee
...
def free_joints_generator(
    ...
    upper_limits = np.minimum(
        get_joint_upper_limits(robot.robot_id, free_joints, robot.physics_client_id),
        np.array(current_positions) + max_distance,
    )
...
def ikfast_inverse_kinematics(
    ...
    for free_positions in generator:
        ...
        for joint_positions in ik_candidates:
            difference = joint_difference_fn(current_joint_positions, joint_positions)
...
def ikfast_closest_inverse_kinematics(
    ...
    if not (
        hyperparameters.max_time < np.inf
        or hyperparameters.max_attempts < np.inf
    ...
    def difference_from_current(joint_positions: JointPositions) -> float:
        ...
        return np.linalg.norm(  # type: ignore
            joint_difference_fn(current_joint_positions, joint_positions), ord=norm
        )
